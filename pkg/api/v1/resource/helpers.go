...
func PodRequestsAndLimitsWithoutOverhead(pod *v1.Pod) (reqs, limits v1.ResourceList) {
	opts := &corev1helpers.PodResourcesOptions{
		ExcludeOverhead: true,
	}
	return corev1helpers.PodRequests(pod, opts), corev1helpers.PodLimits(pod, opts)
}
...
func PodRequestsAndLimitsReuse(pod *v1.Pod, reuseReqs, reuseLimits v1.ResourceList) (reqs, limits v1.ResourceList) {
	return corev1helpers.PodRequests(pod, &corev1helpers.PodResourcesOptions{
			Reuse: reuseReqs,
		}), corev1helpers.PodLimits(pod, &corev1helpers.PodResourcesOptions{
			Reuse: reuseLimits,
		})
}
